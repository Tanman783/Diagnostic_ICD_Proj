...
# 2. CONFIGURATION
EXPERIMENT_NAME = "node2vec_emb_mc_aplasia_45_days_codes_upto_mlp_1layer"
...
for dim in DIMENSIONS:
    ...
    try:
        # UPDATED: Added workers=4 to match 2-layer script
    ...
    retraining_strategy = partial(
        mlp1.retrain_static_mlp, embedding_dict=embedding_dict, vector_size=dim
    )
    
    # D. Run Manager
    fold_results = metrics.run_nested_cv_experiment(
        X=X_raw_df, df_target=df_target, fold_files=fold_files, tuning_callback=tuning_strategy,
        retraining_callback=retraining_strategy, model_name=f"MLP_{NUM_LAYERS}Layer",
        feature_name=f"Node2Vec_{dim}", cohort_name=COHORT_NAME
    )
    
    # Inject Meta-data (STRICT UNIFORMITY)

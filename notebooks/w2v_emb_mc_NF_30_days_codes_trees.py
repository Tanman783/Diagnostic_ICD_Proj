...
# CONFIGURATION
EXPERIMENT_NAME = "w2v_emb_mc_NF_30_days_codes_trees"
...
for dim in VECTOR_SIZES:
    print(f"\n{'='*50}")
    print(f"Processing Word2Vec Dimension {dim}")
    print(f"{'='*50}")
    
    for model_name in MODELS_TO_RUN:
        
        # 1. Tuning Strategy (Trains W2V on the fly)
        tuning_strategy = partial(
            trees.tune_w2v_model,
            model_name=model_name,
            thresholds=THRESHOLDS,
            vector_size=dim
        )
        
        # 2. Retraining Strategy (Trains Final W2V)
        retraining_strategy = partial(
            trees.retrain_w2v_model,
            model_name=model_name,
            vector_size=dim
        )
